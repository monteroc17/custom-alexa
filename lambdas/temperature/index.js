  ...
  return new Promise((resolve, reject) => {
    http.get(API_URL, function(res){
        ...
        res.on('end', function(){
            ...
            let res = data[0].Temperature.Metric.Value + ` degrees. Humidity is at ${data[0].RelativeHumidity} percent.`;
            resolve(res)
        });
    }).on('error', function(e){
...
const GetWeather_Handler =  {
    ...
    async handle(handlerInput) {
        ...
        let say = 'The weather in Santa Clara currently is: ';
        
        let msg = await getWeather(API_URL);
        say += msg;
        console.log('RESPUESTA: ',say);
        return responseBuilder
          .speak(say)
          .reprompt('try again, ' + say)
          .getResponse();
